...
export async function POST(req: NextRequest) {
  try {
    ...
    const fullPrompt = '당신은 삼성전자, LG, 현대자동차, SK 등 국내 주요 대기업에서 10년 이상 서류 전형을 담당해온 인사 전문가입니다. ' +
      ...
      '[1분 자기소개 스크립트 - 잡통 플러스]\n' +
      '이 지원자가 실제 면접장에서 1분 동안 말할 자기소개 스크립트를 작성하세요. 반드시 550~650자 분량으로 작성하세요 (자연스러운 발화 속도 기준 1분 분량이며, 이보다 짧으면 안 됩니다).\n' +
      '반드시 지켜야 할 규칙:\n' +
      '- 이력서·자기소개서에 실제로 있는 경험·성과·수치만 사용하세요. 지어내지 마세요\n' +
      '- 이 지원자만의 가장 강력한 차별화 포인트(다른 지원자와 겹치지 않는, 서류에 드러난 구체적 강점) 하나를 중심으로 구성하세요. 여러 강점을 나열하지 말고 하나에 집중하되, 그 강점을 뒷받침하는 구체적 사례·수치·과정을 충분히 풀어서 분량을 채우세요\n' +
      '- 다음 구조로 작성하세요: (1) 짧은 도입 인사 및 한 문장 핵심 요약(훅) → (2) 그 강점을 보여주는 구체적 경험 하나를 상황-행동-결과 순서로 자세히 서술 (전체 분량의 절반 이상을 여기에 할애) → (3) 그 경험이 지원 직무·회사와 어떻게 연결되는지 → (4) 입사 후 포부로 마무리\n' +
