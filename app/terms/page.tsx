...
1. "서비스"란 회사가 제공하는 서류 전형(자소서·이력서·경력기술서 등) 분석 서비스 및 이와 관련된 제반 서비스를 의미합니다.
...
4. "분석권"이란 서비스 이용을 위해 충전하거나 무료로 제공받은 포인트로, 1분석권 = 1회 서류 분석을 의미합니다.
5. "분석 결과"란 회원이 서비스를 이용하여 생성한 서류 분석 리포트를 말합니다.`,
    },
    {
...
1. 무료 분석: 회원 가입 후 1회 무료 분석 제공 (종합 점수, 전체 총평, 핵심 문제 공개)
2. 서류 전체 분석: 분석권 사용 시 전체 분석 결과 제공 (종합 분석, 문장 개선, 강점 & 조언)
...
5. 기타 회사가 추가 개발하거나 제휴 계약을 통해 제공하는 서비스`,
    ...
    {
      title: '제5조 (서비스 이용 요금)',
      content: `① 서비스의 기본 이용은 무료이며, 회원 가입 시 무료 분석권 2회를 제공합니다.

② 추가 서비스 이용을 위해서는 분석권을 구매해야 합니다. 요금 체계는 다음과 같습니다.
- 1회권: 2,900원 / 1회
- 5회권: 9,900원 / 5회 (1회당 1,980원, 32% 할인)

③ 분석권은 결제일로부터 1개월간 사용 가능합니다. 유효기간이 경과한 미사용 분석권은 소멸됩니다.

④ 1회 서류 분석 시 분석권 1회가 차감됩니다.
...
⑤ 요금은 사전 공지 후 변경될 수 있습니다.`,
    },
    {
      ...
      content: `① 청약철회 (구매 취소)
전자상거래 등에서의 소비자보호에 관한 법률 제17조에 따라, 이용자는 분석권 구매일로부터 7일 이내에 청약철회(구매 취소)를 요청할 수 있습니다. 단, 구매한 분석권을 1회라도 사용한 경우에는 사용한 분량을 제외한 잔여 분석권에 대해서만 환불이 가능합니다.

② 환불 사유
다음 각 호의 경우 환불을 요청할 수 있습니다.
1. 단순 변심: 구매일로부터 7일 이내, 미사용 분석권에 한해 전액 환불
2. 부분 사용: 구매일로부터 7일 이내, 미사용 잔여 분석권에 대해 비례 환불
3. 서비스 장애: 회사 귀책 사유로 인한 서비스 불가 시 미사용 분석권 전액 환불 또는 분석권 복구
4. 오결제: 이용자의 의도와 다르게 결제된 경우 전액 환불

③ 환불 제한 사유
다음 각 호의 경우 환불이 제한됩니다.
1. 구매일로부터 7일이 경과한 경우
2. 구매한 분석권을 전부 사용한 경우
3. 이용약관 위반으로 서비스 이용이 제한된 경우
4. 부정한 방법으로 분석권을 취득한 경우
...
③ AI 모델의 특성상 서류 분석 결과에 오류가 포함될 수 있으며, 회사는 분석 결과의 정확성, 완전성, 합격 보장에 대해 보증하지 않습니다. 이용자는 분석 결과를 참고 자료로 활용하고 최종 판단은 본인이 해야 합니다.
      ...
      <main style={{ maxWidth: 800, margin: '0 auto', padding: '60px 24px 80px' }}>
        ...
        <div style={{ background: 'rgba(230,168,0,0.1)', border: '1px solid rgba(230,168,0,0.3)', borderRadius: 16, padding: '20px 24px', marginBottom: 40 }}>
          <p style={{ fontSize: 14, color: textSub, lineHeight: 1.8, wordBreak: 'keep-all' }}>
            <strong style={{ color: point }}>핵심 요약:</strong> 잡통은 한국 취준생을 위한 서류 전형(자소서·이력서·경력기술서) 분석 서비스입니다. 가입 시 무료 분석 1회를 드리며, 추가 이용은 분석권 구매가 필요합니다. 분석 결과는 합격률 향상을 위한 참고 자료로 활용하시기 바랍니다.
          </p>
        </div>
