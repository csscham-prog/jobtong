...
2. 서비스 제공: 서류 전형(자소서·이력서·경력기술서 등) 분석 서비스 제공, 분석 결과 제공, 맞춤 피드백 제공
...
2. 파기방법: 전자적 파일 형태로 기록·저장된 개인정보는 기록을 재생할 수 없도록 기술적 방법을 사용하여 삭제합니다.`,
    ...
    {
      title: '제8조 (업로드 파일 및 분석 원문의 처리)',
      content: `① 회사는 이용자가 서비스 이용 과정에서 첨부하는 자기소개서, 이력서, 경력기술서, 채용공고 등의 파일 원본을 서버에 저장하지 않습니다. 첨부된 파일은 분석에 필요한 텍스트만 일시적으로 추출한 후, 분석 완료 즉시 서버 메모리에서 삭제됩니다.

② 이용자가 직접 입력하거나 붙여넣은 원문 텍스트 또한 별도로 저장되지 않습니다.

③ 다만, 분석 결과(종합 점수, 총평, 항목별 진단, 개선 제안 등)는 이용자가 마이페이지에서 재확인할 수 있도록 저장되며, 이 결과 안에는 서비스 제공을 위해 원문의 일부 짧은 문장(최대 50자 내외)이 인용될 수 있습니다.

④ 저장된 분석 결과는 본인만 조회할 수 있으며, 제3자에게 제공되지 않습니다.

⑤ 회원 탈퇴 시 위 분석 결과를 포함한 모든 관련 데이터는 즉시 삭제됩니다.`,
    },
    {
