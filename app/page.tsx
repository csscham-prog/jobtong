      ...
      <main style={{ fontFamily: "'Pretendard', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", background: '#f7f6f3', minHeight: '100vh' }}>
        ...
        <section style={{ background: '#f7f6f3', padding: '72px 24px' }}>
          <div style={{ maxWidth: 760, margin: '0 auto' }}>
            <h2 style={{ textAlign: 'center', fontSize: 28, fontWeight: 800, color: '#0f2244', marginBottom: 48, letterSpacing: '-0.5px' }}>
              무료 체험 vs 전체 분석
            </h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
              {/* 무료 */}
              <div style={{ background: '#fff', borderRadius: 20, padding: '36px 32px', border: '1.5px solid #ece9e1' }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#aaa', marginBottom: 12, letterSpacing: 1 }}>무료 체험</div>
                <div style={{ fontSize: 32, fontWeight: 800, color: '#0f2244', marginBottom: 24 }}>₩0</div>
                <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {[
                    ...
                    <li key={item.text} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 14, color: item.ok ? '#333' : '#bbb' }}>
                      <span style={{ fontWeight: 700, color: item.ok ? '#10b981' : '#ddd', fontSize: 16 }}>{item.ok ? '✓' : '✗'}</span>
                      {item.text}
                    </li>
                  ))}
                </ul>
              ...
              <div style={{ background: '#0f2244', borderRadius: 20, padding: '36px 32px', border: '1.5px solid #0f2244', position: 'relative' }}>
                <div style={{ position: 'absolute', top: -14, left: '50%', transform: 'translateX(-50%)', background: '#e6a800', color: '#fff', fontSize: 12, fontWeight: 700, padding: '4px 16px', borderRadius: 20, whiteSpace: 'nowrap' }}>추천</div>
                <div style={{ fontSize: 13, fontWeight: 700, color: 'rgba(255,255,255,0.5)', marginBottom: 8, letterSpacing: 1 }}>전체 분석</div>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 24 }}>
                  <span style={{ fontSize: 32, fontWeight: 800, color: '#fff' }}>₩2,900</span>
                  <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>/ 1회</span>
                </div>
                <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {[
                    '종합 점수',
                    '전체 총평',
                    '핵심 문제 1가지',
                    '항목별 세부 점수 (논리성·구체성·적합성·표현력)',
                    '문장 단위 개선 제안',
                    '"이런 내용 추가하세요" 제안',
                    '잘 된 점 피드백',
                    '최종 종합 조언',
                  ].map(item => (
                    <li key={item} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 14, color: 'rgba(255,255,255,0.85)' }}>
                      <span style={{ fontWeight: 700, color: '#f0c040', fontSize: 16, flexShrink: 0 }}>✓</span>
                      {item}
                    </li>
                  ))}
                </ul>
                <div style={{ marginTop: 28, borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: 20, fontSize: 13, color: 'rgba(255,255,255,0.4)', textAlign: 'center' }}>
                  5회권 ₩9,900 · 1회당 ₩1,980
                </div>
              </div>
            </div>
      ...
      <main style={{ fontFamily: "'Pretendard', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", background: '#f7f6f3', minHeight: '100vh' }}>
        ...
        <div style={{ maxWidth: 680, margin: '0 auto', padding: '48px 24px' }}>
          <div style={{ background: '#fff', borderRadius: 20, padding: '40px 36px', border: '1px solid #ece9e1' }}>
            ...
            <p style={{ color: '#888', fontSize: 14, marginBottom: 36, lineHeight: 1.75 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <p style={{ textAlign: 'center', color: '#aaa', fontSize: 13, lineHeight: 1.7 }}>
                ...
                전체 분석(문장 개선 제안 포함)은 1회권(₩2,900) 또는 5회권(₩9,900)으로 확인하세요.
              </p>
            </div>
          </div>
        </div>
      ...
      <main style={{ fontFamily: "'Pretendard', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", background: '#f7f6f3', minHeight: '100vh' }}>
        ...
        <div style={{ maxWidth: 680, margin: '0 auto', padding: '48px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 20, padding: '40px 36px', border: '1px solid #ece9e1', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 88, fontWeight: 900, color: getScoreColor(result.totalScore), lineHeight: 1, marginBottom: 8 }}>
              {result.totalScore}
            </div>
          ...
          <div style={{ background: '#fff', borderRadius: 20, border: '1px solid #ece9e1', overflow: 'hidden', position: 'relative' }}>
            ...
            {/* 블러 미리보기 */}
            <div style={{ padding: '32px 36px', filter: 'blur(5px)', pointerEvents: 'none', userSelect: 'none', opacity: 0.5 }}>
              ...
              <div style={{ background: '#f7f6f3', borderRadius: 12, padding: '20px' }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#aaa', marginBottom: 8 }}>지원동기</div>
                <div style={{ fontSize: 14, color: '#666', marginBottom: 12 }}>이 부분은 지원 이유가 명확하지 않습니다.</div>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#e6a800', marginBottom: 6 }}>💡 이렇게 고치세요</div>
                <div style={{ fontSize: 14, color: '#333' }}>구체적인 경험과 수치를 활용하여...</div>
              </div>
            </div>

            {/* 잠금 오버레이 */}
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, transparent 0%, rgba(255,255,255,0.97) 35%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', padding: '40px 36px' }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 36, marginBottom: 12 }}>🔒</div>
                <h3 style={{ fontSize: 20, fontWeight: 800, color: '#0f2244', marginBottom: 8 }}>전체 분석 결과 보기</h3>
                <p style={{ color: '#888', fontSize: 14, lineHeight: 1.75, marginBottom: 28 }}>
